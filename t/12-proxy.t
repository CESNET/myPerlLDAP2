#!/usr/bin/perl -w

BEGIN { $| = 1; print "1..6\n";}
#END {print "not ok 1\n" unless $SOK;}

use strict;
use lib qw(../myPerlLDAP2);
use myPerlLDAP::conn;
use myPerlLDAP::entry;
use myPerlLDAP::aci;
use myPerlLDAP::attribute;
use myPerlLDAP::utils qw(:all);
use t::C;
use Data::Dumper;
use vars qw($SOK);

$SOK = 1;
print "ok 1\n" if $SOK;

# Don't warm about missing attribute classes
$myPerlLDAP::attribute::_D=0;

# - 2 -----------------------------------------------------------------------
$SOK = 1;
my $conn = new myPerlLDAP::conn({"host"    => $C::LDAPServerHost,
				 "port"    => $C::LDAPServerPortS,
				 "certdb"  => 1,
				 "bind"    => $C::BindDN,
				 "pswd"    => $C::BindPasswd,
				 "proxydn" => $C::ProxyDN}) or $SOK = 0;
print "not ok 2\n" unless $SOK;
print "ok 2\n" if $SOK;

# - 3 -----------------------------------------------------------------------
$SOK = 1;
my $entry = $conn->read($C::ProxyDN);
$entry->setValues('userPassword', 'eeZo9Vahxeig9adxx');
$conn->update($entry) or $SOK = 0;
print "not ok 3\n" unless $SOK;
print "ok 3\n" if $SOK;

# - 4 -----------------------------------------------------------------------
$SOK = 1;
my $aci = $conn->readACI($C::ProxyDN,
			 #atributy ktery nas zajimaji
			 @C::attrs, 'userPassword') or $SOK = 0;

print "not ok 4\n" unless $SOK;
print "ok 4\n" if $SOK;

# - 5 -----------------------------------------------------------------------
$SOK = 1;
$SOK = 0 unless $aci->e('write');

print "not ok 5\n" unless $SOK;
print "ok 5\n" if $SOK;

# - 6 -----------------------------------------------------------------------
$SOK = 1;
$SOK = 0 unless $aci->a('cn', 'write');

print "not ok 6\n" unless $SOK;
print "ok 6\n" if $SOK;
